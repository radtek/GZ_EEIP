...
<body>
    ...
    <div class="operating-area">
    	...
    	<sys:auth grant="MGT_RVW_MEETING_ADD" username="${sessionScope.loginUser.userName}">
    	<div class="button-content">
    ...
    function view(id){
	    $('.index-iframe' , window.parent.document ).attr('src', '${pageContext.request.contextPath}/meetingMgt/review/meetReviewView.do?id='+id);
	}
    
    function edit(id){
	    $('.index-iframe' , window.parent.document ).attr('src', '${pageContext.request.contextPath}/meetingMgt/review/meetReviewApprove.do?id='+id);
	}
    
    function modify(id){
	    $('.index-iframe' , window.parent.document ).attr('src', '${pageContext.request.contextPath}/meetingMgt/review/meetReviewForm.do?id='+id);
	}
    
    function audit(id){
	    $('.index-iframe' , window.parent.document ).attr('src', '${pageContext.request.contextPath}/meetingMgt/review/meetReviewFileApprove.do?id='+id);
	}
    
    function file(id){
	    $('.index-iframe' , window.parent.document ).attr('src', '${pageContext.request.contextPath}/meetingMgt/review/meetReviewFileView.do?id='+id);
	}
    ...
    function remove(id){
     ...
     $(function(){
    	 ...
    	 $(".add-form-btn").click(function(){
    	    	$('.index-iframe' , window.parent.document ).attr('src','${pageContext.request.contextPath}/meetingMgt/review/meetReviewForm.do');
    	 })
    	...
    	function getOperationCol(data, type, row) {
     		...
     		//}
     		//if( row.status == 1 ){
     			//html += "<a onclick=\"edit('" + row.id + "');\" title=\"审批\"><i class=\"fa fa-edit\"></i></a> "; //审批
     		//}
     		//if( row.status == 4 ){
     		     // html += "<a onclick=\"file('" + row.id + "');\" title=\"文件上传\"><i class=\"fa fa-upload\"></i></a> "; // 文件上传
     		//}
     		//if( row.status == 5 ){
     			//html += "<a onclick=\"audit('" + row.id + "');\" title=\"结果审批\"><i class=\"fa fa-paste\"></i></a> ";//结果审批
     		//}
        ...
        function loadGridData() {
            ...
            var sorting = [ [ 2, 'desc' ] ];
    	      ...
    	      //扩展参数
            var extraParams = [{'name':'type','value':2},{'name':'key','value':searchValue}];
    ...
    <sys:auth grant="MGT_RVW_MEETING_EDIT" username="${sessionScope.loginUser.userName}">
