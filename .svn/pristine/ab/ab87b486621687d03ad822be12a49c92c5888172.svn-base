...
<html>
<head>
	<title>电子物证管理-受理人员操作台-检材受理</title>
  	<meta charset="utf-8">
...
<body>
    <div class="crumbs-nav">
        <p class="crumbs-text">
			当前位置：<a>电子物证管理</a><span>></span><a>受理人员操作台</a><span>></span><a>检材受理</a>
        </p>
    </div>
    ...
    <script>
        ...
        $(function(){
	   	...
	   	function accept(id){
	   		var url = "${pageContext.request.contextPath}/elecEVIDMgt/acceptanceOperatorStation/acceptedForm.do?id="+id;
        	$(".index-iframe", window.parent.document).attr("src",url);
        }

	   	function getOperationCol(data, type, row) {
			var html = "";
			html+="<a class=\"operating-btn\" onclick=\"accept('" + row.id + "');\" title=\"受理\"><i class=\"fa fa-check-square-o\"></i></a>";
			return html;
		}
    	...
    	function loadGridData() {
    			...
    			//扩展参数
    			var extraParams = [{ "name" : "query", "value" : query}, { "name" : "status", "value" : 0}];
