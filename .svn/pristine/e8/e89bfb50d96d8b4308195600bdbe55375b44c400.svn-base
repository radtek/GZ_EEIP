...
<body>		
	<div class="crumbs-nav">
        <p class="crumbs-text">
           	当前位置：<a>会议管理</a><span>></span><a>计划</a><span>></span><a>内审年度计划</a><span>></span><a>新增</a>
        </p>
   	</div>
	...
	<script>
		$(function(){
			...
			if ( "${param.id}" ) {
				$(".crumbs-text").html("当前位置：<a>会议管理</a><span>></span><a>计划</a><span>></span><a>内审年度计划</a><span>></span><a>修改</a>");
			}
		    ...
		    vm.addPlan = function(){
		    	planDataVM.plans.push({"dept":"","element":"","jan":"","feb":"","mar":"","apr":"","may":"","jun":"","jul":"",
		    		"aug":"","sep":"","oct":"","nov":"","dec":"","id":null});
		    }
		    
		    planDataVM.removePlan = function(){
		    	 if(this.id != null){
		    		 $.ajax({
			    		 url:'${pageContext.request.contextPath}/meeting/annualPlanApp/annualPlan/removeAnnualPlanData.do',
			    		 type:'post',
			    		 datatype:'json',
			    		 data:{'id':this.id},
			    		 success:function(){
			...
			vm.backToList = function(){//返回
				 $.ajax({
			  		  url:'${pageContext.request.contextPath}/annualPlanApp/updateAnnualPlanAttStatusData.do',
			  		  traditional: true,
			  		  data:{'ids':removeIds},
			  		  datatype:'json',
			  		  type:'post',
			...
			//数据绑定
			function bindDataVM(){
				planAppDataVM.annualPlans = planDataVM.plans;
				planAppDataVM.title = $("#year").val() + "年度内审年度计划";
				planAppDataVM.status = 1;
				planAppDataVM.creatorName = "${sessionScope.loginUser.userName}";
				planAppDataVM.creatorId = "${sessionScope.loginUser.userId}";
				planAppDataVM.creatorAlias = "${sessionScope.loginUser.userAlias}";
				planAppDataVM.addAtt = addIds;
				planAppDataVM.year = $("#year").val();
			}
		    	...
		    	if( fileNameVal == ""){
					...
					if( $.inArray(fileType,allFileArr)== -1 ){
		        		saveFailure("系统提示：不能上传“"+ fileType +"”这类型文件！");
			  	      	...
			  	      	$.ajaxFileUpload({
			  	      		url:'${pageContext.request.contextPath}/annualPlanApp/uploadAnnualPlanAttData.do',
		 					data:{
			 					'id' : planAppData.id,
	    ...
	    $('.file-list').on('click','.remove',function(){
	  	  ...
	  	  var  filethis = $(this);
	  	  $.ajax({
	  		  url:'${pageContext.request.contextPath}/annualPlanApp/removeAnnualPlanAttData.do',
	  		  data:{'id':fileid},
	  		  datatype:'json',
	  		  type:'post',
	  		  success:function(data){
