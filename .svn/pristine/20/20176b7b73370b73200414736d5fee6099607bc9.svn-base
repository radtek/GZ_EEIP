...
<body>
	<div class="crumbs-nav">
        <p class="crumbs-text">
           	当前位置：<a>现场勘验</a><span>></span><a>查看</a>
        </p>
    </div>
	...
	$(function(){
	    ...
	    for(var i=0;i<inquestDocData.length;i++){
	    	docs += '<p><a href="${pageContext.request.contextPath}/inquest/document/downloadDocumentData.do?id='+inquestDocData[i]['id']+'">'+inquestDocData[i]['name']+'</a>'+
	    			'<button type="button" class="btn btn-color1 download" id="'+inquestDocData[i]['id']+'"><i class="fa fa-download"></i>下载</button></p>'
	    }
	    ...
	    $(".item").on('click','.download',function(){
	    	var id = $(this).attr('id');
	    	window.location.href = "${pageContext.request.contextPath}/inquest/document/downloadDocumentData.do?id="+id;
	    })
