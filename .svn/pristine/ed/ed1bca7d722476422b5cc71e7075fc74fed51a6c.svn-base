...
<body>
    <div class="crumbs-nav">
        <p class="crumbs-text">
			当前位置：<a>知识管理</a><span>></span><a>知识文件管理</a>
        </p>
    </div>
    ...
    function editDocument(id){
    	var url = "${pageContext.request.contextPath}/knowledgeMgt/fileMgt/fileMgtModifyForm.do?id="+id;
    	$(".index-iframe", window.parent.document).attr("src",url);
    }
    
    function updateDocument(id){
    	var url = "${pageContext.request.contextPath}/knowledgeMgt/fileMgt/fileMgtUpdateForm.do?id="+id;
    	$(".index-iframe", window.parent.document).attr("src",url);
    }
    ...
    $(function(){
    	...
    	function getOperationCol(data, type, row) {
     		var html = "";
     		if( row.type == 1 ){
     			html += "<a onclick=\"updateDocument('" + row.id + "');\" title=\"修改\"><i class=\"fa fa-edit\"></i></a> "; //修改
     			//html += "<a onclick=\"viewDocument('" + row.id + "');\" title=\"查看\"><i class=\"fa fa-search\"></i></a> "; //查看
     			html += "<a title='删除' data-toggle=\"modal\" data-target=\"#sysNoticePopup\" onclick=\"setRemovedFileParam('" + row.id + "', '" + row.name + "','"+ row.type +"');\"><i class='fa fa-times'></i></a>";
     		}else if( row.status == 3 ){
     			if( row.docId != null && row.docId != "" ){
     				html += "<a onclick=\"editDocument('" + row.id + "');\" title=\"修改\"><i class=\"fa fa-edit\"></i></a> "; //修改"
     			}else{
        ...
        function loadGridData() {
      	  //数据列绑定
    	      var columns = [
    	          ...
    	          { "mData" : null , "bSortable" : false,  "mRender": getOperationCol, "sWidth": "8em" }
            ];
            ...
            dataTable.initialize({
                ...
                "dataSource" : "${pageContext.request.contextPath}/tmpDocOperationLog/getAllGirdData.do",
     ...
     function setRemovedFileParam(id, name, type) {
      	  $("#rmId").val(id);
  	      $("#rmName").val(name);
  	      $("#rmType").val(type);
  	      setSystemNoticeContent("是否确认删除知识文件&nbsp;"+$("#rmName").val()+"&nbsp;?");
     }
