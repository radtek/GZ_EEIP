...
<body>
    ...
    <div class="form-content">
        <form class="form-horizontal">
             <div class="form-group">
                ...
                <div class="col-sm-4">
                    <input type="text" placeholder="" class="form-control" data-bind = "value:sn" id="orginalSn" required/>
                </div>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-4">
                    <select class="form-control dirSelect" id="dirId" required disabled>
    ...
    <script>
	    ...
	    var documentData = loadData("${pageContext.request.contextPath}/system/assessibility/document/loadDocumentInfoData.do", { "id" : "${param.id}" });
	    console.log(documentData);
	    //var documents = documentData.documents;
	    //$("#orginalSn").val(documentData.sn);
	    var docData = loadData("${pageContext.request.contextPath}/tmpDocOperationLog/loadTmpDocOperationLogData.do", { "id" : "" });
	    var docRelationships = docData.docRelationships;
	    docData.sn = documentData.sn;
	    docData.year = documentData.year;
	    docData.name = documentData.name;
	    ...
	    for(var i=0;i<dirData.length;i++){
	    	if ( dirData[i].id == documentData.dirId ) {
	    		dirHtml += '<option value="'+dirData[i]['id']+'" selected>'+dirData[i]['name']+'</option>';
	    	} else {
	    ...
	    var fileTypeName = documentData.extensions.split(".")[documentData.extensions.split(".").length-1];
		var dataName = documentData.name;
		var dataId = documentData.id;
		...
		html += '<p><a href="${pageContext.request.contextPath}/tmpDocOperationLog/downloadData.do?id='+dataId+'">'+dataName+'</a></p>';
		html += '</div>';
		html += '</div>';
	    
	    /* var uploadHtml = '<div class="ccflie"><div class="col-sm-2"></div><div class="col-sm-10">'
            +'<a href="${pageContext.request.contextPath}/tmpDocOperationLog/downloadData.do?id='+documentData.id+'"><span id="fileNameText">'+documentData.name +'</span></a>'
            +'</div></div>'; */
        $('.recordfile').html(html);
        //$(".input-text").val(documentData.name);	
	    ...
	    function bindDataToVM(){
	    	...
	    	vm.dirId = $(".dirSelect").val();
	    	
	    	if( vm.path ) {
	    		vm.path = documentData.path;
	    		vm.extensions = documentData.extensions;
	    	}
	    	console.log(docRelationships);
	    }
        ...
        //复写
		if ( documentData != null && documentData.documents.length > 0 ) {
			for ( var i=0; i < documents.length; i++) {
				...
				docRelationships.push({'id':null,'srcDocId':'','relevantDocId':documents[i]['id']});
			}
		}
