...
<body>
	<div class="crumbs-nav">
        <p class="crumbs-text">
           	当前位置：<a>会议管理</a><span>></span><a>会议申请</a><span>></span><a>普通会议</a><span>></span><a>新增</a>
        </p>
   	</div>
	<div class="form-content">	
		<form class="form-horizontal">														
                ...
                <div class="col-sm-10">
                    <div class="associated-file-content">
                    	<ul class="associated-file-list list-width-name">
                    		<!-- <li><p>02</p><span><i class="fa fa-times"></i></span></li> -->
                    	</ul>
			...
			<div class="form-group">
				...
				<div class="col-sm-4" data-bind="with:meetingAppVM">
					<input type="text" id="startDate" class="form-control datetimepicker datetimepicker-before" data-bind="dateTimePicker:startDate" required/>
				</div>
				<label class="col-sm-2 control-label"><span class="required-fields">*</span>结束时间：</label>
				<div class="col-sm-4" data-bind="with:meetingAppVM">
					<input type="text" id="endDate" class="form-control datetimepicker datetimepicker-after" data-bind="dateTimePicker:endDate" required/>
				</div>					
			</div>
			<div class="form-group">
...
<script type="text/javascript">
	...
	if( url.indexOf("id=") != -1 ){
	  $(".crumbs-text").html("当前位置：<a>会议管理</a><span>></span><a>会议申请</a><span>></span><a>普通会议</a><span>></span><a>修改</a>");
	}
		...
		$(".file-list").on("click",".add",function() {
			...
			if( $(".associated-file-list").find("#" + id).text() == "undefined" || $(".associated-file-list").find("#" + id).text() == ""){
				$(".associated-file-list").append('<li><p id="' + id + '"> '+ alias
						                   +'</p><span class="remove" aria-hidden="true" data-id="'+ id+'" title="删除"><i class="fa fa-times"></i></span>'
						                   +'</li>');
				...
				$("button").attr("disabled", false);
	  			...
	  			if( length == 0 ){
	  				...
	  				if (saveData("${pageContext.request.contextPath}/meeting/meetingApp/saveMeetingAppData.do",meetingAppDataVM)) {
						saveSuccess("${pageContext.request.contextPath}/meetingMgt/generalMeet/meetingApplyList.do");
					} else {
						saveFailure();
					}
	  			}
